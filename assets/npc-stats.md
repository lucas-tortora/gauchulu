# NPC STAT BLOCKS — LA TIERRA MALA
*CoC 7e. Argentine pampas, 1821. Print and keep beside the dice.*
*Organised by arc. Entities at the end.*

**PC spells from the Mandinga sequence** (post-C3, if triggered): see `assets/mandinga-sequence.md` for full mechanics. Spells available: Mengua (Shrivelling), Marca del Dueño (Warding), Espanto (Implant Fear), Voz del Patrón (Dominate), Cuero (Flesh Ward). Add to the relevant PC's sheet after the sequence runs.

---

## WEAPON NOTES — 1821 ARGENTINA

| Weapon | Skill | Damage | Notes |
|---|---|---|---|
| Facón (large gaucho knife) | Fighting | 1D6+2+DB | Standard gaucho weapon |
| Cuchillo de campo | Fighting | 1D4+2+DB | Shorter field knife |
| Boleadoras | Fighting | 1D4+DB | On success: target DEX roll or entangled/prone |
| Lanza (lance, mounted) | Fighting | 1D8+1+DB | Mounted only |
| Spear (foot) | Fighting | 1D6+1+DB | — |
| Flintlock pistol | Firearms | 1D10 | 1 shot; reload 3 full rounds |
| Flintlock musket/rifle | Firearms | 2D6 | 1 shot; reload 4 full rounds; range 50/75 |
| Saber | Fighting | 1D8+1+DB | — |
| Improvised | Fighting | 1D4+DB | Blunt objects, tools |

**Damage Bonus table**: STR+SIZ 2–64: −2 | 65–84: −1 | 85–124: +0 | 125–164: +1D4 | 165–204: +1D6

---

## ARC 1 — LA TIERRA MALA (C1–C3)

---

### DON EUSEBIO VALDEZ
*62. Cattle rancher. Client, culprit, necessary.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 50 | 50 | 65 | 40 | 75 | 55 | 75 | 60 | 11 | +0 | 0 | 5 |

**Skills**: Persuade 65%, Intimidate 60%, Fast Talk 55%, Firearms (Pistol) 50%, Ride 65%, Accounting 70%, History 55%, Library Use 50%, Spot Hidden 55%

**Weapons**: Flintlock pistol 50% (1D10); belt knife 40% (1D4+2)

**Notes**: He does not want to fight. He wants to control — through money, through pressure, through plausible deniability. If cornered with no option, he draws the pistol. His hands shake. He has not shot at a person in twenty years. The ink stains on his right hand affect fine motor control: Firearms skill drops to 35% by C4.

---

### TOMÁS ARREDONDO
*43. Foreman. Afraid, loyal, cracking.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 65 | 65 | 70 | 55 | 55 | 50 | 40 | 45 | 13 | +1D4 | 1 | 8 |

**Skills**: Ride 70%, Track 55%, Natural World 60%, Fighting (Brawl) 55%, Fighting (Facón) 60%, First Aid 40%, Intimidate 45%, Spot Hidden 50%

**Weapons**: Facón 60% (1D6+2+1D4)

**Notes**: He will not attack investigators unless he believes they are directly threatening his family. If they are: he is fast, strong, and has been using a knife since he was twelve. His hands stop trembling when he draws.

---

### RANCH HAND (generic)
*Estancia worker. Loyal by economic necessity.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 60 | 60 | 60 | 55 | 45 | 45 | 12 | +0 | 0 | 8 |

**Skills**: Fighting (Brawl) 45%, Fighting (Facón) 50%, Ride 60%, Track 40%

**Weapons**: Facón 50% (1D6+2); possibly musket 35% (2D6, 1 shot)

**Notes**: They follow Don Eusebio's orders. They don't know why. Some of them have seen the lights in the south pasture and crossed themselves and said nothing. They will not die for their employer. They will bluster and follow and fall back.

---

### KUYEN
*52. Mapuche. Ritual leader. Eight months without stopping.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 50 | 65 | 55 | 65 | 85 | 95 | 70 | 60 | 12 | +0 | 0 | 8 |

**Magic Points**: 19  
**Sanity**: 50 (has spent heavily maintaining the ritual over months)

**Skills**: Occult 85%, Cthulhu Mythos 45%, Spot Hidden 80%, Listen 75%, Track 65%, Ride 70%, First Aid 65%, Persuade 70%, Psychology 75%, Natural World 80%, Language (Mapudungun) 100%, Language (Spanish) 70%, Fighting (Knife) 45%

**Weapons**: Short knife 45% (1D4+1). She does not seek combat. Her hands are needed for symbol work.

**Ritual capability**: She can deploy and maintain the binding working, redraw disrupted lines under pressure, and read the entity's geometric approach. Her Occult skill functions at +1 difficulty in conditions she has not encountered before; at normal difficulty within her own tradition.

**Spells**

### Elder Sign
**Cost**: 3 MP  
**Casting time**: 1 round (she draws it in ochre or ash)

Kuyen draws the Sign on a surface — ground, stone, a wooden post, a person's palm. Mythos entities must make a POW roll to cross it or come within 1 meter of it. Failure: they cannot approach that round and must reroute. Success: they cross, but with visible effort — the geometry distorts around the Sign as they pass through it.

**At the ritual**: She uses this at breach points — the north marker that shifted in R2, the anchor position before the PC plants there, the ground around Nahuel when he collapses. Drawing takes her full action for that round. She cannot redraw symbol lines and cast Elder Sign in the same round.

**Limit**: The Sign holds as long as the ochre is intact. The Hound eats the certainty of pigment lines — an Elder Sign drawn in ochre near the entity's position will fade within 1D3 rounds as the Hound's presence degrades it. Stone-carved or burned marks last longer but she does not have those tools tonight.

---

### Voorish Sign
**Cost**: 1 MP  
**Casting time**: Instantaneous

Forces partially visible or concealed entities into full visibility for 1D6 rounds. During this window, the entity's exact position in the space is readable — not its intent, not its next move, but where it is and where its edges are.

**At the ritual**: Kuyen uses this to read the Hound's movement vectors. When she casts it, she turns and looks at the eastern darkness for one beat before returning to her symbol work — a small, sharp movement that investigators may notice. She uses what she sees to decide where to redraw lines.

**For a Marked PC**: If Kuyen casts Voorish Sign and a Marked PC is watching the same space, the Marked PC receives what she receives — the entity's geometry becomes fully readable for that window. They do not need to make a Spot Hidden roll. They know exactly where it is and which angle it is about to use. One round of warning, certain rather than probable.

**For uninstructed investigators**: They see Kuyen look sharply at something that doesn't appear to be there, then immediately redirect her symbol work. They can follow her gaze and attempt Spot Hidden (Regular) to catch the edge of what she saw.

---

### Bless Blade
**Cost**: 1D6 MP  
**Casting time**: 1 round

One weapon — knife, brand, whatever is at hand — becomes capable of harming entities otherwise immune to physical damage. The blessing lasts for one successful strike, then is spent.

**At the ritual**: Kuyen will not cast this on herself — her hands are needed for the diagram. She will cast it on an investigator's weapon if they hand it to her and she has a round to spare. She does not explain what she is doing. She takes the weapon, draws a mark along the blade in ochre from her belt pouch, and hands it back. The mark glows faintly for approximately one round, then fades to ordinary pigment whether the strike lands or not.

**What it means at this table**: The Hound cannot be killed. But a Blessed weapon that connects will force a disruption — the entity must make a CON roll or be pushed back 1D6 meters and lose its next action reestablishing position. The same effect as the goat's headbutt, but available to a PC. Once per casting, once per combat at most — Kuyen does not have the MP to chain this.

**Limitation**: Kuyen must spend a full action to cast it. She cannot bless a blade and redraw a line in the same round. This is a decision that costs the ritual something. She will only make it if she judges the threat severe enough to warrant the trade.

---

**Notes**: She will fight if Nahuel is directly threatened — not tactically, instinctively, throwing herself between. Otherwise she does not stop the symbol work under any circumstance. Hitting her is like breaking the only instrument you have.

---

### NAHUEL
*24. Ritual focus. Three weeks in. Running on nothing.*

**Normal stats** (for reference; not currently accessible):

| STR | CON | SIZ | DEX | INT | POW |
|---|---|---|---|---|---|
| 60 | 70 | 60 | 65 | 70 | 75 |

**Current state — C3 and forward:**

| Effective STR | Effective CON | Effective DEX | Effective POW | Current HP | Current MP |
|---|---|---|---|---|---|
| 35 | 40 | 35 | 40 | 4 | 8 |

*All physical and mental stats halved or worse from sustained ritual focus. He is not ill — he is depleted. There is a difference. Illness recovers. This has to be rebuilt.*

**Notes**: He cannot fight. He can barely stand without support. His value in the ritual is enormous; his value in any physical situation is zero. He knows this. A hand to grip is worth more to him than any weapon.

**Recovery**: Full rest + food + no ritual contact: recover 1 point of each depleted stat per day. He will not be functional for combat for weeks.

---

### ROSA QUIROGA
*71. Elder. Guardian of memory. Immovable.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 30 | 55 | 50 | 35 | 85 | 90 | 65 | 55 | 10 | −1 | −1 | 4 |

**Magic Points**: 18  
**Sanity**: 65 (high — she has been watching this for 14 months and made her peace)

**Skills**: Occult 75%, Spot Hidden 70%, Listen 80%, Psychology 80%, Persuade 65%, Language (Mapudungun) 85%, Language (Spanish) 90%, Natural World 70%, History (local) 80%

**Weapons**: None she would use. If pressed absolutely: grinding stone as improvised 25% (1D6−1).

**Notes**: She does not fight. She does not run. She stands in her doorway and the doorframe binding is intact and she says what she has to say. The only situation in which she becomes physically dangerous is if someone touches her silver ornament — she will not permit this, and she is stronger than she looks when she has decided something.

---

### CATA (Catalina)
*Late teens. Farmer's daughter. Currently bound at the eastern gap.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 50 | 55 | 55 | 60 | 55 | 50 | 11 | +0 | 0 | 8 |

**Current condition — bound, after hours of kneeling**:
- Effective STR for escape: 50 (circulation restricted, poor leverage from kneeling)
- Escape rolls: R1 STR Hard (25%); R2 STR Regular (45%); R3 SAN 0/1D4 first, then STR Easy (75%)

**Skills**: Natural World 40%, Animal Handling 55%, Ride 45%, First Aid 35%

**Weapons**: None. If she frees herself she runs. If physically restrained after freeing herself she fights — scratching, biting, headbutting — at Fighting (Brawl) 30%, but she is small and in poor condition. She is not trying to win. She is trying to make it too costly to hold her.

**Notes**: She is not a coward. She has been quiet for hours because screaming accomplished nothing. She is waiting. See TABLE-STATE.md — Cata's escape attempts for full R1/R2/R3 mechanics.

---

### TRIBE MEMBER (generic, active)
*Tehuelche/Mapuche. Kuyen's people. Mid-thirties to mid-fifties.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 60 | 60 | 60 | 60 | 60 | 65 | 12 | +0 | 0 | 8 |

**Skills**: Fighting (Spear) 55%, Fighting (Brawl) 50%, Ride 65%, Track 70%, Natural World 75%, Occult (tribal tradition) 40%, Spot Hidden 60%

**Weapons**: Spear 55% (1D6+1); short knife 50% (1D4+2)

**Notes**: They follow Kuyen. They do not act without her direction except to protect her. The ones who survive R1 are the west-position member and one diagonal. They are not hostile to investigators who are not hostile to Kuyen. They will restrain Cata if Kuyen orders it.

---

## ARC 2 — LA SALAMANCA (C4–C6)

---

### SILVIO MÉNDEZ
*38. Wrong Returned. Estancia foreman. Holds himself back.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 65 | — | 60 | 50 | 55 | 20 | — | +0 | 0 | 7 |

**Special — Wrong Returned physiology**:
- HP: does not track normally. His body persists past damage that should stop it.
- Wounds close — not heal, close. The tissue reorganizes.
- He does not flinch, gasp, or stagger from physical damage. He registers it and continues.
- He can be knocked prone (STR vs STR), restrained (opposed STR), or rendered unconscious by head trauma (Hard CON roll after any blow to the head, fail = unconscious for 1D6 rounds). He cannot be killed by conventional means.
- Fire affects him normally — he feels it, flinches from it, and his reorganized tissue does not close burn wounds cleanly.
- At 0 HP by conventional measure: he is functional but visibly wrong. Wounds open. His movement becomes geometric rather than organic — straight lines, precise turns.

**Skills**: Fighting (Brawl) 55%, Fighting (Facón) 60%, Ride 65%, Track 60%, Agriculture 70%, Spot Hidden 50%

**Weapons**: Facón 60% (1D6+2). He does not draw it unless directly threatened. He is trying to remain normal.

**Notes**: He does not want to hurt anyone. He wants to complete the work and go home to Mendoza. The entity is pulling him toward the pit. He holds himself back with what remains of his will. If investigators treat him with genuine respect — as a skilled worker, not a horror — he will tell them everything he knows.

---

### HÉCTOR RÍOS
*54. Wrong Returned. Former San Ruiz farmer. Fully oriented.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 70 | — | 70 | 50 | 65 | 15 | — | +1D4 | 1 | 7 |

**Special**: Same Wrong Returned physiology as Silvio. Héctor is more physically powerful and less conflicted — he has accepted what he is more completely. This makes him calmer and, if necessary, more dangerous.

**Skills**: Fighting (Brawl) 60%, Track 65%, Natural World 70%, Spot Hidden 75% (geometry-aligned)

**Weapons**: Whatever is at hand. He does not carry weapons deliberately.

**Notes**: He goes directly to significant ritual objects and touches them without hesitation. He is the calmest person in any ritual space. This is unsettling. He is not violent unless protecting the geometry itself.

---

### MARTA RÍOS
*43. Wrong Returned. Former teacher and midwife. Most articulate of the three.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 45 | — | 50 | 55 | 75 | 20 | — | +0 | 0 | 7 |

**Special**: Same Wrong Returned physiology. Marta retains more of her original personality than Héctor — moments of warmth surface before the flatness returns. This makes her more useful in conversation and slightly more unpredictable in crisis.

**Skills**: First Aid 60%, Psychology 65%, Persuade 55%, Natural World 60%, Spot Hidden 70% (geometry-aligned), Language (Mapudungun, partial) 40%

**Notes**: She is the one most likely to explain what is happening in terms investigators can understand. She is also the one who will co-anchor the C9 center ring if rapport was built. That is her closing action — not a combat action.

---

### ALDAO QUIRCE
*48. Scholar, network operative. Brilliant, morally misaligned.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 45 | 50 | 60 | 55 | 90 | 65 | 95 | 70 | 11 | +0 | 0 | 8 |

**Skills**: Occult 75%, Library Use 85%, History 80%, Cthulhu Mythos 40%, Persuade 75%, Psychology 65%, Spot Hidden 65%, Astronomy 70%, Mathematics 80%, Dodge 50%

**Weapons**: He does not carry a weapon. If cornered: brass theodolite as improvised club 30% (1D6−1). He will flee before fighting. He is quick and knows the terrain of any building he has spent time in.

**Notes**: He is not brave. He is calculating. If investigators have something he needs, he will negotiate. If they have nothing he needs and are threatening him, he runs. He has survived forty years of dangerous scholarship by being faster than the situation.

---

### QUIRCE'S ESCORT (×3, generic)

*Hired guards, Buenos Aires. Competent, unpaid-for heroics. Also the template for El Patrón's anonymous human machinery: the Counter (C10), the House Cleaners (C11), and the Weak Point 1 ambushers + false vigilante (C12 — vigilante swaps Intimidate 55% for Disguise 50%). One stat block, one labor market.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 65 | 60 | 65 | 55 | 50 | 45 | 12 | +1D4 | 1 | 8 |

**Skills**: Fighting (Brawl) 60%, Fighting (Knife) 55%, Firearms (Pistol) 50%, Dodge 45%, Intimidate 55%, Spot Hidden 45%, Ride 50%, Stealth 40%

**Weapons**: Flintlock pistol 50% (1D10, 1 shot, slow reload) · cuchillo 55% (1D4+2+1D4) · fists 60% (1D3+1D4)

**Morale (C6 escort)**: one of the three down → the other two disengage, covering Quirce's extraction. They fight to wound and extract, never to kill, and will not follow anyone toward the pit after dark: *"Eso no lo pagó."* — *"He didn't pay for that."*
**Morale (C11 cleaners)**: fight to escape with the satchel, not to win.
**Morale (C12 ambush)**: three down → the rest scatter into the side streets.

---

### THE GREAT WIZARD OF SALAMANCA
*Late old age. Cult head. Genuinely believes.*

| STR | CON | SIZ | DEX | INT | POW | EDU | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|
| 35 | 45 | 55 | 35 | 95 | 120 | 100 | 10 | +0 | 0 | 4 |

**Magic Points**: 24 (regains 1D3 MP per full round spent in contact with the ochre floor)  
**Sanity**: 0 (long since gone, replaced by certainty)  
**Armor**: None — **chamber geometry** (see below)  
**Initiative**: DEX 35 — he almost never acts first in melee. In his chamber he may **pre-empt** one hostile spell per scene (see *Deflection of the Ochre*).

**Skills**: Cthulhu Mythos 70%, Occult 90%, Library Use 85%, History 80%, Persuade 75%, Psychology 70%, Dodge 25%, Language (Latin) 90%, Language (Quechua) 65%

**Chamber geometry (passive, always on in his study)**:
- Hostile spells cast *by anyone standing on the ochre floor* are one difficulty step harder to cast.
- Hostile spells cast *at the Wizard* while he stands on the ochre floor are one difficulty step harder to land (caster's roll, or POW resist as appropriate).
- Physical attacks against him are one difficulty step harder (distance subtly wrong).
- His own geometry-manipulation spells are one difficulty step easier.

**Spells known** (full list for chamber confrontation):

| Spell | MP | Effect (summary) |
|---|---|---|
| **Deflection of the Ochre** *(chamber)* | 3 | **Counterspell vs Shrivelling and other destructive body-targeting magic.** See chapter-05 chamber scene. |
| **Veil of Not-Quite** *(illusion)* | 4 | One target sees the Wizard where he stood 2 seconds ago. Opposed POW; on failure, target acts against the afterimage for 1 round. |
| **Mental Suggestion** | 3 | *"Put it down. We are not finished."* One sentence, one command. Opposed POW; on failure, comply 1 round (cannot attack; may speak). |
| **Mindblast** | 6 | 1 target, 2D6 SAN loss (no POW resist if cast on ochre floor). Buys 1–2 rounds. |
| **Dread** | 3 | 1D3 targets within 10 m; CON roll or flee 1D3 rounds toward the nearest tunnel. |
| **Opening the Way** | 8 | Opens the secondary seal behind the desk (escape crawl-space). One round to complete; interruptible. |
| **Shrivelling** | 6 | He *has* it; will not cast first — reserves for desperate self-defense only. |
| **Contact Entity** (variant) | 10 | Not used in C5 — reserved for Coypac. |

**Weapons**: None physical. Brass ink-knife on the desk: 25%, 1D4−2, only if grappled.

**Notes**: He does not want a duel. Goal is **conversation → misdirection → escape**. If reduced below 5 HP or out of MP, he flees through the crawl-space regardless. He is the slowest person in any physical confrontation — DEX 35 — but the chamber fights for him.

---

## ARC 3–4 — THE PIT / BUENOS AIRES (C7–C12)

---

### CULT PRIEST OF THE GREAT SLEEPER (captured)

*Fifties. Banda Oriental accent. Ash-blond beard. Calm is the weapon. Captured by the Cursed Gaucho two days before C7; carried east as proof the cult is operational now.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|
| 55 | 60 | 55 | 50 | 70 | 80 | 11 | +0 | 0 | 7 |

**Skills**: Cthulhu Mythos 30%, Occult 65%, Persuade 60%, Stealth 50%, Listen 55%, Sleight of Hand 45% (the slipped wrist — C7 Eastern Probe alt-encounter)

**Spells (if ungagged ≥2 rounds)**: a 12-syllable call — no mechanical effect at the pit, but every Marked PC recognizes the rhythm from the whispering tunnels (C5): SAN 0/1, and a free clue — the cult chants in the first people's meter.

**Carries**: the coded eight-name list (C7 — handout H6 in handouts-c5-c7.md). Teeth show the Saens-skull wear pattern: first-people communion paste (Hard Cthulhu Mythos to identify; Mercedes will want to study it).

**SAN** when he speaks of the Great Sleeper: 0/1D4. He is calm. He believes what he says. The belief is the unsettling part.

---

### THE CURSED GAUCHO
*Mid-thirties. Three years hunting. The most dangerous human in the campaign.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 75 | 70 | 65 | 80 | 65 | 60 | 40 | 45 | 13 | +1D4 | 1 | 9 |

**Skills**: Fighting (Knife/Facón) 85%, Fighting (Brawl) 70%, Firearms (Pistol) 65%, Dodge 75%, Stealth 70%, Track 80%, Ride 80%, Survival 75%, Spot Hidden 75%, Listen 70%, Psychology 55%

**Weapons**:
- Facón 85% (1D6+2+1D4) — his primary; he has drawn it eight times in three years
- Cuchillo de campo 80% (1D4+2+1D4) — left hand, off-hand
- Flintlock pistol 65% (1D10, 1 shot) — not loaded. He carries it for what it represents. He will load it if he has three minutes and privacy.

**Combat notes**: He fights with two knives, the facón in his dominant hand and the cuchillo as deflect-and-counter. His Dodge reflects three years of staying alive in ambush situations. He does not telegraph. If he has decided to kill someone, they are probably already dead before they realize it has begun.

**The one constraint**: He will not fight in a way that risks his target reaching El Patrón before him. He has been three years building toward one ending. He will not have it taken.

---

### CAPTAIN JUAN PABLO GALÍNDEZ
*49. Retired cavalry officer. The operation's spine.*

| STR | CON | SIZ | DEX | INT | POW | EDU | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 65 | 60 | 65 | 60 | 75 | 60 | 70 | 55 | 12 | +1D4 | 1 | 8 |

**Skills**: Fighting (Saber) 70%, Firearms (Pistol) 65%, Dodge 55%, Ride 75%, Navigate 60%, Command 70%, Strategy 65%, Spot Hidden 65%, Psychology 60%, First Aid 45%

**Weapons**:
- Saber 70% (1D8+1+1D4) — worn at the belt, never decorative
- Cavalry pistol 65% (1D10, 1 shot)
- Belt knife 55% (1D4+2+1D4)

**Notes**: He is precise, calm, and does not waste motion. He has been in situations where the plan fell apart before — he has a fallback for the fallback. His combat style is conservative: he holds ground, waits for openings, does not commit to a killing attack until it is certain. At 49 he knows he is slower than he was at 30. He plans around this.

---

### CAPITÁN ETTORE BORGHI
*Late 40s. Genoese merchant sailor. Paid and professional.*

| STR | CON | SIZ | DEX | INT | POW | EDU | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|
| 65 | 65 | 60 | 60 | 65 | 55 | 55 | 12 | +0 | 0 | 8 |

**Skills**: Navigate 75%, Pilot Boat 80%, Fighting (Brawl) 60%, Firearms (Pistol) 55%, Spot Hidden 65%, Climb 65%, Swim 70%

**Weapons**: Flintlock pistol 55% (1D10, 1 shot); belaying pin 60% (1D6+DB)

**Notes**: He will fight if the ship or his crew is threatened. He will not fight for someone else's argument. He has four oarsmen who follow his lead — if he draws, they draw.

---

## ANTAGONIST

---

### EL PATRÓN — HUMAN GUISE
*Don Eligio Dadálah. Avatar of Nyarlathotep. Has been in this body since 1798.*

| STR | CON | SIZ | DEX | INT | POW | APP | HP | DB | Build | Move |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 55 | 75 | 60 | 65 | 100 | 150+ | 60 | 13 | +0 | 0 | 8 |

**APP note**: 60 on paper, but APP is effectively 0 for memorability — the face will not hold in the mind. Players who try to describe him afterward find they cannot. He is the right height, the right age, wearing the right clothes, and nothing else registers.

**Magic Points**: 30 (in this form; replenishes)  
**Sanity loss**: 1/1D6 to realize what you are speaking to; 1D10/1D100 for any glimpse of what is behind the face

**Skills**: All social skills at 90%+. Cthulhu Mythos 100%. Spot Hidden 95%. Psychology 95%. He reads people the way Kuyen reads geometry.

**Spells**: Functionally unlimited. He does not cast spells at investigators — it is beneath the framing he has constructed. He negotiates. He offers. He leaves a sharpened pencil on the chair.

**Cannot truly be killed in this form.** Reducing HP to 0: the body drops and is found later, cause of death unclear. He is elsewhere before the body cools.

**Notes**: He is not encountered in combat in the normal sense. The campaign's endings are the only places he can be dealt with — Hidden, Used, or Destroyed — and none of them require a Fighting roll. The investigators' weapon against him is the choice they make at the dock. Not a saber.

---

## ENTITIES

---

### LA LLORONA — DISPLACED / ENRAGED
*Dolores Quiroga's psychic remnant. No longer on circuit. Moving east.*

**Not physical** — conventional weapons pass through her. The following stats govern interactions where resistance matters.

| POW | MP | Coherence | Move |
|---|---|---|---|
| 85 | 17 | 4 (see below) | 6 / passes through solid matter |

**Coherence**: Her ability to maintain form. Not HP — disruption. Each significant disruption (goat headbutt, fire directed at her position, salt line she must cross) reduces Coherence by 1. At 0: she disperses. She reconstitutes in 1D6 hours, further east.

**Her attacks**:

| Attack | Effect |
|---|---|
| **Pass-through** (passive) | Any person she moves through: SAN 1/1D6 and receives a flash of Dolores's final memory (fire; child in corner; the eastern wall). No resistance possible. |
| **Focused passage** (enraged) | Deliberately passes through a target. Opposed POW — her POW 85 vs target POW×5. On win: SAN 1/1D6 + target must make INT roll or be stunned 1 round, living the memory. |
| **Presence** | Sustained proximity (more than 3 rounds): SAN 0/1D4 per additional round from the weight of unresolved grief. |

**What affects her**:
- **The goat**: Direct headbutt reduces Coherence by 2. Disrupts form visibly. Forces retreat.
- **Fire**: Reduces Coherence by 1 if directed at her (burning brand, not ambient firelight).
- **Salt line / protective marks**: She cannot cross them passively — she must spend 1 MP and make a POW roll to cross. She will not cross Rosa's doorframe binding at all.
- **A Marked PC**: Can read her geometric state (is she coalescing? dispersing? approaching?) as spatial fact, giving one round of warning before a pass-through.
- **Weapons**: Pass through. No effect.

**Current trajectory**: Moving east toward the burial pit. She is not hunting investigators. She is following the pull of unfinished geometry. Investigators who are between her and the pit may be passed through incidentally.

---

### LA CABRA (THE GOAT)
*Mandinga's marker. Threshold being. Looks like a brown-and-white goat.*

| STR | CON | SIZ | DEX | INT | POW | HP | DB | Build | Move | MP |
|---|---|---|---|---|---|---|---|---|---|---|
| 80 | 90 | 40 | 95 | 80 | 115 | 13 | +0 | 0 | 10 | 23 |

**SAN loss**: 0/1D6 — not for her appearance, which is ordinary. For the moment an investigator realizes what they are looking at.

**Skills**: Fighting (Gore) 85%, Spot Hidden 95%, Listen 95%, Stealth 100%, Track (geometric) 100%

**Stealth and Track notes**: These never fail. She makes no sound — no hooves, no breathing, no impact. Her tracking is not by scent or sight; she reads geometry and is already where it requires her to be. The Move stat governs her when she is visibly running in a scene. The *preceding* quality is not Move — it is something else, and there is no roll for it.

---

**Attacks**

| Attack | Skill | Damage / Effect |
|---|---|---|
| **Gore — physical targets** | Fighting 85% | 1D6+DB |
| **Disrupt — supernatural entities** | Fighting 85% to connect; then contested POW | See below |

**Disrupt (supernatural entities only)**:

The goat must first connect — Fighting (Gore) 85%. A miss is a miss: she ran at something that moved or the angle changed.

On a hit, the targeted entity makes a contested POW roll: both sides roll POW×5, compare success levels. Goat POW is 115 (target: 99% capped). Many entities also roll near cap — the contest is genuine, not automatic.

| Result | Effect |
|---|---|
| Goat wins | Coherence −1D3; entity pushed back 1D6 meters; form disrupts visibly |
| Tie | Physical contact confirmed. No disruption. She bounces off something that barely noticed. |
| Entity wins | She impacts but the entity's geometry absorbs it. No effect. She lands, shakes her head, looks at it. |

**Note on the Hound specifically**: Average Hound POW is 120 — higher than the goat's 115. In a contested roll at these levels, the Hound wins roughly half the time. She is powerful but she is not the answer to the Hound. She is a delay and a disruption, not a solution.

---

**Damage and resilience**

She is not invincible on paper. She has HP 13.

- **Conventional weapons**: Her body closes around wounds. Each round she takes no action, she heals 2 HP. Weapons do normal damage but she fights through it without changing behavior.
- **Fire**: Bypasses the closing-around quality. Fire damage does not heal. This is the one thing that actually accumulates on her.
- **The Hound's ichor**: Affects her normally — she can be ichor-damaged and must wipe it off like anyone else. Her CON is high enough that she is unlikely to die from it quickly, but she is not immune.
- **The Hound's tongue attack (POW drain)**: Contests her POW of 115. On loss, she loses POW as anyone does. Significant POW loss would reduce her Disrupt effectiveness in subsequent contested rolls.

At 0 HP: she goes down. She does not die — Mandinga's investment in her is not so fragile — but she is inert for 1D6 hours, then rises. If investigators leave her behind while she is inert, she will be ahead of them when she wakes.

---

**Her judgment**

She acts when she decides to act. She cannot be directed, commanded, or pointed at a target. She will not Gore on command. She will not Disrupt on request. She times her interventions and her timing is always correct — she does not act too early and she does not act too late. When she moves, it is because the moment has arrived.

This means the Keeper rolls for her, not in response to player instructions. Players can position themselves near her, watch her, notice when she tenses — but they cannot tell her what to do. The deal-holder PC has slightly more weight with her than others, but even they cannot command. She is not a weapon. She is a party with her own interests that happen to align.

---

*Cross-reference: `assets/entity-hound.md` for El Sabueso stats. `TABLE-STATE.md` for Cata's escape mechanics, goat deal terms, and La Llorona's current trajectory.*
